...
hdir=`pwd`
#qhaDirs="qha_4.575a"
qhaDirs="qha_4.658285a qha_4.685a qha_4.730a qha_4.759a qha_4.801a"
...
for j in $qhaDirs; do 
  cd $j
  rm done_already
  for k in `echo disp-*`; do if [[ $(tail -n 1 $k/OUTCAR | awk '{print $1}') == "Voluntary" ]]; then echo $k  >> done_already ; else echo $k ; fi; done  2>/dev/null > partial_submitted
  sleep 3
  ...
  for i in $dispList; do 
    ...
    cp ../../qhaSUBstuff/* .
    cp ../../qha129.pbs .
    sed -i 's/xxNAMExx/p129-'$j'-'$i'/g' qha129.pbs
    echo "im in" `pwd` 
    qsub qha129.pbs
    cd $hdir
  done
...
echo "Submitted jobs:"
cat partial_submitted_all
echo ""
echo "Total number of jobs submitted now is:"
wc -w partial_submitted_all
